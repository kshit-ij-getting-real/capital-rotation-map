...
export function TreemapClient({ initialData }: { initialData: TreemapResponse }) {
  ...
  const [sizeMetric, setSizeMetric] = useState<'weight' | 'marketCap' | 'tradedValue'>('marketCap');
    ...
    return {
      tooltip: {
        formatter: (p: any) => {
          ...
          return `${d.ticker} - ${d.name}<br/>Sector: ${d.group}<br/>Last: ${d.lastPrice.toFixed(2)}<br/>1D: ${d.returns['1D'].toFixed(2)}%<br/>1W: ${d.returns['1W'].toFixed(2)}%<br/>1M: ${d.returns['1M'].toFixed(2)}%<br/>Weight: ${d.size.toFixed(3)}<br/>Updated: ${new Date(d.timestamp).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })}`;
        },
      },
      visualMap: {
        min: data.legend.min,
  ...
  }, [data, search, showLabels]);

  return (
    <div className="container">
      <h2>India Sector Rotation Map</h2>
      <p className="small">Last updated: {new Date(data.updatedAt).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })}</p>
